...
function reloadChatBrowser(chatName) {
  Array.from(document.querySelector(".chatBrowser").children).forEach(
    (element) => {
      element.parentNode.removeChild(element);
    }
  );
...
function chatIsScrolledBottom() {
  ...
  let $browser = $(browser);
  return (
    browser.scrollHeight - $browser.scrollTop() - $browser.outerHeight() < 1
  );
}
...
function searchNewChats() {
  ...
  document
    .querySelectorAll(".newChatNameWrapper")
