...
function editbutton(el_id, data_type = "text") {
  ...
  Array.from(document.getElementsByClassName("editButton")).forEach((element) => {element.style.display = "none";});
}
function undoEditDel() {
  Array.from(document.getElementsByClassName("editButton")).forEach((element) => {element.style.display = "inline";});
...
function flipped(field) {
  ...
  $.ajax({
    ...
    type: "POST",
    data: {field: field,value: newVal,csrfmiddlewaretoken: $("input[name=csrfmiddlewaretoken]").val()},
    dataType: "json",
    success: function (data) {
      if (!data.success) {document.getElementById(field).children[0].children[0].checked = !newVal;}
    },
  });
}
...
function sendVal(field, val, isText = false) {
  $.ajax({
    ...
    success: function (data) {
      if (data.success) {
        if (isText) {
          document.getElementById(field).children[0].innerHTML = sanitize(val);
        } else {
          document.getElementById(field).children[0].innerHTML = val;
        }
        undoEditDel();
      }
    },
...
function newProfile(){$('.hiddenPicForm input[type=file]').click();}
